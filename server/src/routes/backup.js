...
backupRouter.get("/", async (req, res, next) => {
  try {
    ...
    const listas = await db
      .prepare(
        "SELECT * FROM lists WHERE household_id = ? AND archived = 0 ORDER BY kind, name",
      )
  ...
  `);
    ...
    for (const l of listas) {
      comItens.push({
        ...
        emoji: l.emoji,
        reusable: !!l.reusable,
        items: (await itensDa.all(l.id)).map((i) => ({
          name: i.name,
          qty: i.qty,
          unit: i.unit,
          category: i.category,
...
const criarLista = db.prepare(
  "INSERT INTO lists (household_id, name, kind, emoji, reusable, created_by) VALUES (?, ?, ?, ?, ?, ?) RETURNING *",
);
...
backupRouter.post("/restore", async (req, res, next) => {
  try {
    ...
    if (Number(dump.version) !== VERSAO) {
      return res
        .status(400)
        .json({ error: "este arquivo nao e um backup desta versao do app" });
    }
    if (!Array.isArray(dump.lists))
      return res.status(400).json({ error: "backup sem listas" });

    let itens = 0;
    let listas = 0;
    let precos = 0;

    const restaurar = db.transaction(async () => {
      for (const origem of dump.lists) {
        const nome = String(origem?.name || "").trim();
        if (!nome) continue;
        let destino;
        if (origem.kind === "general") {
          destino = await getGeneralList(req.user.householdId);
        } else {
          destino = await acharLista.get(req.user.householdId, nome);
          if (!destino) {
            destino = await criarLista.get(
        ...
        for (const item of origem.items || []) {
          ...
          if (await itemIgual.get(destino.id, nomeItem, unidade)) continue;
          posicao += 1;
          await inserirItem.run({
            listId: destino.id,
            productId: item.matchKey
              ? ((await acharProduto.get(item.matchKey))?.id ?? null)
              : null,
            name: nomeItem,
            ...
            addedBy: req.user.id,
            priceSnapshot:
              typeof item.priceSnapshot === "string"
                ? item.priceSnapshot
                : null,
      ...
      for (const linha of Array.isArray(dump.history) ? dump.history : []) {
        ...
        if (await historicoIgual.get(chave, quando, preco)) continue;
        await inserirHistorico.run(
          (await acharProduto.get(chave))?.id ?? null,
          chave,
